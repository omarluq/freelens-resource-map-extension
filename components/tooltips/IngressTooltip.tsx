  ...
  renderPaths(ingress: Renderer.K8sApi.Ingress) {
    ...
    return rules.map(
      (
        rule: {
          host?: string;
          http?: { paths: Array<{ path?: string; backend: IIngressBackend }> };
        },
        index: number,
      ) => {
        return (
          <div className="rules" key={`rule-${rule.host || index}`}>
            {rule.host && (
            ...
            {rule.http && (
              <Renderer.Component.Table className="paths">
                {rule.http.paths.map(
                  (path: { path?: string; backend: IIngressBackend }, index: number) => {
                    const { serviceName, servicePort } = this.getBackendServiceNamePort(
                      path.backend,
                    );
                    ...
                    return (
                      <Renderer.Component.TableRow
                        key={`path-${path.path || serviceName}-${index}`}
                      >
                        <Renderer.Component.TableCell className="path">
                          {path.path || ""}
